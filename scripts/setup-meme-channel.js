#!/usr/bin/env node
/**
 * Rebrand the channel (Zero Yen Otaku, formerly Mr. WorldWideWebster) as the broke NEET who makes fan edits for his
...
const CHANNEL_ID = 'UC2_aNJgemODOCsOLsYwaelg';  // the channel keeps its id through renames
...
const KEYWORDS = ['zero yen otaku', 'acapella', 'anime edit', 'amv', 'kpop edit', 'anime acapella', 'kpop acapella', 'anime opening',
...
(async () => {
  ...
  if (ch.id !== CHANNEL_ID) {
    throw new Error('That is not the Zero Yen Otaku (ex Mr. WorldWideWebster) channel; stopping.');
  }
